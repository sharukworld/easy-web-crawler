            ...
            if (!error && response.statusCode == 200) {
                ...
                that.isLinkAlreadyScrapped(linkArray).then(isScrapped => {
                    that.upsertStore(linkArray, isScrapped).then(upserted => {
                         that.popHighestScoredUrl().then(url => {
                             that.addUrlToVisitedList(url);
                             let vistedStoreName = ApplicationConstant.VISITED_STORE_NAME + that.seedUrl;
                             that.db.SCARD(vistedStoreName, (err, number) => {
                                if(number < that.numberOfUrlToSeed) {
                                    that.crawler(url);
                                }
                                else {
                               that.db.SMEMBERS(vistedStoreName, (err, list) => {
